...
- **`start_dashboard.sh` / `stop_dashboard.sh`** (repo root) wrap the phone-facing `cpx_dashboard.py --gpio --http-port 80` invocation: pre-flight checks refuse to start if already running or if port 80 / `/dev/ttyACM0` is already held by something else, start backgrounds it with `nohup` + confirms it's actually streaming (not just that the process exists) before printing the phone URL, stop matches on the script name alone (not an exact flag list — fragile the moment the start command's flags change, and there's only ever one instance since the serial port can't be shared).
